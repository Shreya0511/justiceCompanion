...
function RequestsPage() {
  ...
  const getAllRequests = async () => {
    const response = await fetch(
      `${process.env.REACT_APP_SERVER}/api/v1/requests/lawyerRequests/` +
  ...
  const acceptRequest = async (id) => {
    console.log("acp:", id);
    const response = await fetch(
      `${process.env.REACT_APP_SERVER}/api/v1/requests/acceptRequest/` + id,
      {
        method: "PATCH",
        headers: {
  ...
  const rejectRequest = async (id) => {
    const response = await fetch(
      `${process.env.REACT_APP_SERVER}/api/v1/requests/rejectRequest/` + id,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
  ...
  const deleteRequest = async (id) => {
    const response = await fetch(
      `${process.env.REACT_APP_SERVER}/api/v1/requests/deleteRequest/` + id,
      {
        method: "delete",
        headers: {
          "Content-Type": "application/json",
  ...
  const revokeRequest = async (id) => {
    const response = await fetch(
      `${process.env.REACT_APP_SERVER}/api/v1/requests/revokeRequest/` + id,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
              ...
              return (
                <div className="request-request" key={index}>
                  ...
                  <div className="request-request-details">
                    <div className="request-user-name">{data.name}</div>
                    <div className="request-user-email">{data.email}</div>
                    <div className="request-buttons">
                      <button
                        className="request-button-accept"
                        onClick={() => {
                          del
                            ? revokeRequest(data._id)
                            : acceptRequest(data._id);
                        }}
                      >
                        {del ? `Revoke ${data.type} request` : "Accept"}
                      </button>
                      <button
      ...
      <div className="requests-page-body">
        <div className="search-section">
          ...
          <div className="filter">
            ...
            />
            <div className={formVisible ? "filter-form" : "hide-element"}></div>
          </div>
        </div>
