...
function PeerconnectChat() {
  ...
  async function query(data) {
    ...
    const response = await fetch(
      ...
      {
        headers: {
          Authorization: "Bearer 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",
        },
        method: "POST",
  ...
  const queryAI = async (message) => {
    try {
      const response = await query({
        inputs: message,
        parameters: { candidate_labels: ["sensitiveInfo"] },
      });
      return response; // Assuming the response format matches your requirement
    } catch (error) {
      throw new Error("Error querying AI:", error);
    }
  };
