...
function FindLawyer() {
  ...
  useEffect(() => {
    const fetchLawyers = async () => {
      const response = await fetch(
        `${process.env.REACT_APP_SERVER}/api/v1/users/getLawyers`,
        {
          method: "get",
          headers: {
  ...
  const getAllRequests = async () => {
    ...
    const response = await fetch(
      `${process.env.REACT_APP_SERVER}/api/v1/requests/userRequests/` +
  ...
  const sendRequest = async (index, type) => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_SERVER}/api/v1/requests/createRequest`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleDeleteRequest = async (index, type) => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_SERVER}/api/v1/requests/deleteRequest/${request._id}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
  ...
  const renderMatchingRequest = (index, type) => {
    ...
    const sendFunction = type === "Chat" ? sendChatRequest : sendHireRequest;
    return matchingRequest.requested ? (
        ...
        {matchingRequest.pending ? (
          ...
          <>
            {matchingRequest.accepted ? (
              ...
              >
                {type === "Chat" ? "Start Chatting" : "Hire Request Accepted"}
              </button>
            ) : (
              <button
      ...
      <button className="request-button" onClick={() => sendFunction(index)}>
        Send {type} Request
      </button>
    );
  };
                ...
                <div className="lawyer-card" key={index}>
                  ...
                  <div className="lawyer-info">
                    ...
                    <div className="lawyer-buttons">
                      {renderMatchingRequest(index, "Chat")}
                      {renderMatchingRequest(index, "Hire")}
                    </div>
                  </div>
                  {lawyer.takesProBono && (
